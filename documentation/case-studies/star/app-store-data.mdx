...
excerpt: 'Project I led as a Product Manager to consistently and reliably access canonical app store data values'
...
- Some sellers include malformed `bundle`’s either from legacy sales pipelines or on accident. A lot of this data is set manually in an ad server by a person, sometimes they’re just copying and pasting from a spreadsheet, there’s plenty of room for accidents.
...
To make matters more complicated, many `storeUrl`’s may be associated with one `bundle` (malformed or canonical). This can make it difficult to programmatically assign relationships accurately since determining the correct `bundle` and `storeUrl` association can be challenging. This is especially true when the `bundle` is an arbitrary ID. For example, if we see that requests for `bundle` `154157` can contain any one of several `storeUrls`, it’s hard to programmatically determine the correct `storeUrl` with any reasonable accuracy.
...
This product was created alongside another product for managing publisher identities. While that tool enabled other teams to solve publisher identity resolution issues (instead of product managers and engineers), it didn’t necessarily help the users figure out which specific publisher should be mapped to a `bundle`. It also didn’t suggest potentially missing pub/bundle mappings.
...
<GlobalCallout
  message='
**Fun fact**
I built this POC in Python, persisted the data in a text file, and submitted a PR to the corporate GitHub account (it’s probably still there) that explained how it works and shared supporting confluence docs. The VP of ENG reviewed it and in turn paired me with some dedicated engineers to build the full version in Go.
'
  type='success'
/>
