...
The required formatting accepted by this tool matched the output generated by the app store data product my team also created so that sensible associations could be inferred based on existing associations and exported from that product and then reinvested into the publisher identity management product.
